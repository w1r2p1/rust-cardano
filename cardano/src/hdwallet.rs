...
struct TestVector {
    ...
    fn check_derivation(test_index: usize, test: &TestVector) {
        ...
        let mut ref_xprv_bytes = [0;XPRV_SIZE];
        ref_xprv_bytes[0..64].clone_from_slice(&test.xPriv[0..64]);
        ref_xprv_bytes[64..96].clone_from_slice(&test.xPriv[96..]);
        let ref_xprv = XPrv::from_bytes_verified(ref_xprv_bytes)
            .expect(&format!("expected a valid private key (test {}, last: 0b{:08b}, first 0b{:08b})", test_index, ref_xprv_bytes[31], ref_xprv_bytes[0]));
        assert_eq!(ref_xprv, xprv, "xprv from test {}", test_index);
...
const TEST_VECTORS : [TestVector;14] =
    ...
    , TestVector {
        ...
        xPriv: & [ 155, 90, 61, 154, 76, 96, 188, 212, 155, 182, 75, 114, 192, 130, 177, 100, 49, 77, 15, 97, 216, 66, 242, 87, 95, 209, 212, 251, 48, 162, 138, 12, 176, 147, 227, 118, 244, 30, 183, 191, 128, 171, 205
                 , 0, 115, 165, 36, 85, 210, 91, 93, 33, 129, 91, 199, 88, 229, 246, 248, 21, 54, 174, 222, 187, 121, 252, 129, 84, 85, 75, 151, 228, 197, 110, 242, 249, 219, 180, 193, 66, 31, 241, 149, 9, 104, 137, 49
                 , 161, 233, 100, 189, 165, 222, 192, 241, 159, 71, 162, 66, 113, 59, 209, 134, 8, 35, 17, 71, 192, 102, 182, 8, 59, 252, 30, 144, 102, 254, 201, 246, 33, 132, 76, 132, 254, 214, 34, 138, 52],
